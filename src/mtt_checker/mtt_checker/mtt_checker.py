...
class MttTimingAnalizer:
    ...
    # next line
    l2_line: int = 0
    l2_rel_time: float = 0.0 # r_{i, j-1}
...
def analyze(p_i, d_i):
    ...
    for idx, ln in enumerate(whole_mtt):
        ...
        if idx + 1 >= len(whole_mtt):
            print_line(ana, p_i, d_i, "one before last line")
            continue
        ana.l2_line = whole_mtt[idx + 1].line
        ana.l2_rel_time = stamp_to_sec(whole_mtt[idx + 1].org_stamp)
        ana.l2_pub_time = stamp_to_sec(whole_mtt[idx + 1].pub_stamp)
        # analize
        ...
        for t in ana.deadline_timer:
            if t <= ana.rel_time:
                ana.cur_ng += 1
                ana.ng += 1
                print_deadline_to(ana, t, "deadline timer timeout")
                c += 1
        c = len(ana.deadline_timer) - c
        if c > 0:
            ana.cur_ok += 1
            ana.ok += 1
            c -= 1
        ...
        for t in np.arange(ana.rel_time + p_i, ana.l2_rel_time, p_i):
            if t + d_i <= ana.l2_rel_time:
                ...
                """
                    ...
                    ana.cur_p_ng += 1
                    ana.p_ng += 1
                    print_periodic(ana, t, p_i, d_i, "periodic+deadline presumed TIMEOUT")
                """
            else:
                print_periodic(ana, t, p_i, d_i, "periodic+deadline START")
                ana.deadline_timer.append(t + d_i)
    cnt = 0
...
def print_analyze(ana, p_i, d_i, desc):
    print(f"{LOC()}|    |{ana.next_rel_time:6f}|{ana.next_rel_time - ana.rel_time:6f}|{ana.next_rel_time + d_i:6f}|        |        |{len(ana.deadline_timer):3}|{ana.ok=:3}|{ana.p_ok=:3}|{ana.ng=:3}|{ana.p_ng=:3}|{desc}|")
    ana.stime.append(ana.next_rel_time)
    
def print_periodic(ana, t, p_i, d_i, desc):
    print(f"{LOC()}|    |{t:6f}|{t - ana.rel_time:6f}|{t + d_i:6f}|        |        |{len(ana.deadline_timer):3}|{ana.ok=:3}|{ana.p_ok=:3}|{ana.ng=:3}|{ana.p_ng=:3}|{desc}|")
    ana.stime.append(t)

def print_deadline_to(ana, t, desc):
    print(f"{LOC()}|    |                 |        |{t:.6f}|        |        |{len(ana.deadline_timer):3}|{ana.ok=:3}|{ana.p_ok=:3}|{ana.ng=:3}|{ana.p_ng=:3}|{desc}|")
    ana.stime.append(t)
    
def print_desc(ana, desc):
    print(f"{LOC()}|    |                 |        |                 |                 |{len(ana.deadline_timer):3}|{ana.ok=:3}|{ana.p_ok=:3}|{ana.ng=:3}|{ana.p_ng=:3}|{desc}|")
