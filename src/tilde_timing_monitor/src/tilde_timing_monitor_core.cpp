  ...
  TildePathConfig & pinfo, double & pub_time, double & cur_ros, double & response_time)
{
  dbg_info_->log(
    fmt::format("|{:.6f}|[{}]:{} <{}> {:.6f} L{} OK={} NG={} dead={}", 
      ...
      dbg_info_->getValidTopicCounter(pinfo),
      dbg_info_->getOkCounter(pinfo), dbg_info_->getNgCounter(pinfo),
      pinfo.deadline_timer.size()));

  if (isOverDeadline(pinfo, pub_time, cur_ros, response_time)) {
    dbg_info_->log(
      fmt::format("--[{}]:{} <{}> L{} DEADLINE OVER OK={} NG={} cur_j={} completed_j={}", 
        __func__, __LINE__, pinfo.path_name.c_str(), dbg_info_->getValidTopicCounter(pinfo),
        dbg_info_->getOkCounter(pinfo), dbg_info_->getNgCounter(pinfo),
...
void TildeTimingMonitor::restartTimers(TildePathConfig & pinfo, double & cur_ros)
{
  ...
  dbg_info_->log(
    fmt::format(
      ...
      pinfo.path_name.c_str(), dbg_info_->getValidTopicCounter(pinfo),
      dbg_info_->getOkCounter(pinfo), dbg_info_->getNgCounter(pinfo),
      pinfo.periodic_timer_val, pinfo.cur_j, pinfo.completed_j));
}
