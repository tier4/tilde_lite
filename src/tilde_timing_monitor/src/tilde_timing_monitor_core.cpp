  ...
  const MessageTrackingTag::ConstSharedPtr msg, TildePathConfig & pinfo)
{
  ...
  log(fmt::format(
    ...
    pinfo.path_name.c_str(), pinfo.r_i_j_1, pinfo.recv_count, pinfo.OK, pinfo.NG,
...
void TildeTimingMonitor::cmdShowStatis()
{
  ...
  for (auto & pinfo : required_paths_map_.at(params_.mode)) {
    ...
    std::cout << fs.c_str() << std::endl;
    fs = fmt::format(
      "response time({}) min={} ave={} max={} (sec)", pinfo.response_time.getCnt(),
      pinfo.response_time.getMin(), pinfo.response_time.getAve(), pinfo.response_time.getMax());
    std::cout << fs.c_str() << std::endl;
    fs = fmt::format(
    ...
    std::cout << fs.c_str() << std::endl;
    fs = fmt::format(
      "topic({}) HZ min={} ave={} max={} (sec) d_i over={} per limit={}", pinfo.hz.getCnt(),
      pinfo.hz.getMin(), pinfo.hz.getAve(), pinfo.hz.getMax(), pinfo.hz.getOver(),
      pinfo.sub_interval.getPerLimit());
    std::cout << fs.c_str() << std::endl;
    fs = fmt::format(
    ...
    std::cout << fs.c_str() << std::endl;
    fs = fmt::format(
      "commnication delay({}) min={} ave={} max={} (sec)", pinfo.com_delay.getCnt(),
      pinfo.com_delay.getMin(), pinfo.com_delay.getAve(), pinfo.com_delay.getMax());
    std::cout << fs.c_str() << std::endl;
